...
const CharInfo = (props) => {
    ...
    const [loading, setLoading] = useState(false); // свойство обьекта идёт загрузка или нет
    const [error, setError] = useState(false); // добавляем вывод спинера об ошибке связи с сайтом (404)
...
/*     useEffect((charId, prevProps) => {
    ...
    const updateChar = () => {
        ...
        onCharLoading(); // показываем спинер загрузки пока загружается инфо по персонажу
        marvelService
            .getCharacter(charId)
            .then(onCharLoaded)
            .catch(onError); 
